...
| win5 | 96マスグリッド(16列×6行)・モーラ表示・BPM連動点灯・Standard/Groupモード・グリッド下にword表示 | なし |
| win6 | BPMメーター(デフォ120)・16stepシーケンサー・スウィング・Tempo/Handモード | なし |
| win7 | カラーパレット選択（10種）・全ウィンドウに一括適用 | なし |
| win8 | モーラフロー（縦書き390スロット・win5カーソル連動書き込み・単語accent表示・ブロックノイズ） | なし |
...
8. → win8: `grid-data`（同じデータ、win5と同期用）

## grid-dataの形式
各セルは `{ mora, word, rms, f0, sc }` のオブジェクト。空白セルは `{ mora: '', word: '', rms: 0, f0: 0, sc: 0 }`。
- win5の `buildGrid` は `typeof item === 'object' ? item.mora : item` で互換処理あり
...
## win5 グリッド下のword表示
- 現在点灯しているモーラに対応するwordをグリッド下に表示
- Standard/Groupモード両方対応
- tick時に `cellWords[cursor]` を参照して `#word-display` に表示
- stop時にクリア
...
## win5→win8 カーソル同期
- tick時にwin5が現在のカーソルインデックスをmainに送信（`grid-cursor` IPC）
- main → win8へ転送
- win8はself-tickをやめ、受け取ったインデックスでgridDataを参照して表示
- Standard/Groupモード両方で完全同期
...
ネット接続必要。フォントはwin9で一括切り替え。

## win8 Flowモーラ表示
- 縦書き26列×15行=390スロットに、win5カーソル連動で1モーラ順次書き込み
- スロットカーソルはリセットなし（json更新後も続きから上書き）
- 単語が切り替わるたびにword-displayが固定位置（やや左上）に表示（accent色・bg2背景ブロック）
- grid_count=0スキップ（空白セルはスキップして表示しない）
- 起動時にmainキャッシュからパレットを取得して適用（`get-palette` IPC）

### win8 word表示
- 位置: 固定（左上 WORD_BLOCK_X, WORD_BLOCK_Y）
- ブロックサイズ: 固定（WORD_BLOCK_W × WORD_BLOCK_H）
- wordがブロックからはみ出してもOK（overflow: visible）
- mix-blend-modeなし（通常の上重なり）
- word-layerのz-index: 2（mora-layerの1より前）
- 調整定数（renderer.js先頭）: `WORD_FONT_SIZE=64` / `WORD_BLOCK_W=220` / `WORD_BLOCK_H=90` / `WORD_BLOCK_X=30` / `WORD_BLOCK_Y=30` / `WORD_TEXT_COLOR='var(--accent)'` / `WORD_BG_COLOR='var(--bg2)'`
...
- canvasはウィンドウリサイズ時に自動的にサイズ更新（resizeイベント対応）
- 調整定数（renderer.js先頭）:
